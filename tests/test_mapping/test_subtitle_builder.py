...
class TestSubtitleBuilder:
    ...
    def test_build_is_pure_rendering_and_preserves_logical_cues(self):
        """Builder must not merge, split, renumber, or mutate finalized events."""
        builder = SubtitleBuilder(rule=SubtitleRule(min_duration=1.0))
        events = [
            SubtitleEvent(index=7, start=0.0, end=0.3, text="大"),
            SubtitleEvent(index=9, start=0.4, end=0.7, text="家好"),
        ]
        before = copy.deepcopy(events)

        rendered = builder.build_to_string(events, fmt="srt")

        assert "大" in rendered
        assert "家好" in rendered
