...
// 小时无位数上限（写出不封顶，1000+ 小时也要能读回）；毫秒超出 3 位截断
export function parseTimestamp(text) {
  const m = /^(?:(\d+):)?(\d{1,2}):(\d{1,2})[,.](\d+)$/.exec(String(text).trim());
  if (!m) return null;
...
// 写出前早失败：非有限时间（NaN/Infinity）会写出 "NaN:NaN:NaN,NaN" 这类无法读回的时间戳。
// 负时间仍由 splitHms / Math.max 静默钳 0（界面依赖该行为）。
function assertFiniteTime(t, label) {
  if (!Number.isFinite(t)) {
    throw new RangeError(`${label}：时间不是有限数值（${t}）`);
  }
}

export function formatSrtTime(t) {
  assertFiniteTime(t, 'formatSrtTime');
  const { h, m, s, ms } = splitHms(t);
  return `${pad(h)}:${pad(m)}:${pad(s)},${pad(ms, 3)}`;
}

export function formatVttTime(t) {
  assertFiniteTime(t, 'formatVttTime');
  const { h, m, s, ms } = splitHms(t);
  return `${pad(h)}:${pad(m)}:${pad(s)}.${pad(ms, 3)}`;
}
...
export function formatAssTime(t) {
  assertFiniteTime(t, 'formatAssTime');
...
// 行内编辑用：接受 m:ss.mmm / mm:ss / h:mm:ss[.mmm] / 纯秒数；非法返回 null
export function parseFlexibleTime(text) {
  ...
  // h:mm:ss（无毫秒）：手打完整时分秒是常见输入，parseTimestamp 要求必须有小数部分
  m = /^(\d+):(\d{1,2}):(\d{1,2}(?:[.,]\d{1,3})?)$/.exec(raw);
  if (m) {
