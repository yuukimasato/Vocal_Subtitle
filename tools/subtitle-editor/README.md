...
2. 点击「打开字幕」或拖入 SRT/VTT/ASS 字幕（也可以从零开始：把播放头移到目标位置按 `N` 或点「插入」，新建后直接进入文本输入）；
3. 波形上每条字幕一个色块：拖左右边缘改入/出点，拖中间整体移动，点击空白定位；
4. 双击右侧列表行编辑文本与时间（在时间框和文本框之间切换焦点不会退出编辑；焦点离开该行或按 Enter 提交）；播放时当前句自动高亮滚动；
5. 「导出 SRT / VTT / ASS」下载结果；编辑过程自动存草稿（localStorage），下次打开同名字幕时可恢复。

**面板大小可调**：拖动视频与波形之间的横条调整波形区高度，拖动波形与字幕列表之间的竖条调整列表宽度；双击分隔条复位，布局自动记忆。
