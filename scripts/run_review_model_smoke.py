...
from vocal_subtitle.asr.window_execution import (
    CancellationToken,
...
def _percentile(values: list[float], percentile: float) -> float | None:
    ...
    index = max(
        0, min(len(ordered) - 1, int(np.ceil(len(ordered) * percentile / 100)) - 1)
    )
...
def _window_smoke(
    engine: Any, audio: np.ndarray, sample_rate: int, args: argparse.Namespace
) -> dict[str, Any]:
    windows = (
        ReviewWindow("smoke-0", 0.0, min(1.0, len(audio) / sample_rate), ()),
        ReviewWindow(
            "smoke-1",
            min(1.0, len(audio) / sample_rate),
            min(2.0, len(audio) / sample_rate),
            (),
        ),
    )
...
def _smoke_engine(
    ...
    *,
    ...
    if window_engine is not None:
        try:
            window_smoke = _window_smoke(window_engine, audio, sample_rate, args)
...
def _smoke_faster_whisper(
    audio: np.ndarray, sample_rate: int, args: argparse.Namespace
) -> dict[str, Any]:
    model = args.faster_whisper_model
    if not importlib.util.find_spec("faster_whisper"):
        return _blocked(
            "faster-whisper", model, "dependency_unavailable: faster_whisper"
        )
    ...
    if not args.allow_download and model_path is None:
        return _blocked(
            "faster-whisper", model, "local_model_path_required", download_allowed=False
        )
...
def _smoke_funasr(
    ...
    if local_model is None and not args.allow_download:
        return _blocked(
            "funasr", model, "local_model_snapshot_missing", download_allowed=False
        )
...
def _smoke_qwen(
    ...
    if availability.get("status") != "ready":
        return _blocked(
            "qwen",
            model or "qwen3-asr",
            availability.get("reason", "unavailable"),
    ...
    return _smoke_engine(
        ...
        lambda: True,
        lambda: list(
            adapter.review(audio, sample_rate, window, language=args.language)
        ),
        audio,
        sample_rate,
        args,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--audio", type=Path, default=ROOT / "test/golden/repeated_phrase_me.wav"
    )
    ...
    parser.add_argument(
        "--whisper-cpp-bin",
        default=str(ROOT / "cache/whisper_cpp/build/bin/whisper-cli"),
    )
    parser.add_argument(
        "--whisper-cpp-model-path",
        default=str(ROOT / "cache/whisper_cpp/models/ggml-tiny.bin"),
    )
    parser.add_argument(
        "--qwen-model-path", default=os.environ.get("QWEN_ASR_MODEL_PATH", "")
    )
    parser.add_argument("--funasr-model", default=os.environ.get("FUNASR_MODEL", ""))
    parser.add_argument(
        "--faster-whisper-model",
        default=os.environ.get("FASTER_WHISPER_MODEL", "large-v3"),
    )
    ...
    print(
        json.dumps(
            {"status": report["status"], "results": results},
            ensure_ascii=False,
            indent=2,
        )
    )
