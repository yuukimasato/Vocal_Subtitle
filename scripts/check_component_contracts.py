...
def check_file(path: Path) -> list[str]:
    ...
    parts = path.parts
    is_domain = (
        "vocal_subtitle" in parts
        and len(parts) > parts.index("vocal_subtitle") + 1
        and parts[parts.index("vocal_subtitle") + 1] in DOMAIN_PACKAGES
    )
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            ...
            if is_service and is_pipeline_import(node):
                violations.append(
                    f"service-pipeline-import:{path}:{node.lineno}:{name}"
                )
        ...
        if is_domain and isinstance(node, ast.Call):
            if (
                ...
                violations.append(
                    f"private-context-call:{path}:{node.lineno}:context.{node.func.attr}"
                )
        ...
        if is_service and isinstance(node, ast.Call):
            if isinstance(node.func, ast.Attribute) and isinstance(
                ...
                if node.func.value.id == "context" and node.func.attr.startswith("_"):
                    violations.append(
                        f"service-private-context-call:{path}:{node.lineno}:context.{node.func.attr}"
                    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--report",
        action="store_true",
        help="Print violations and always exit successfully",
    )
