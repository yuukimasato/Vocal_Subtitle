...
def inspect_feedback_profile(config: Any) -> dict[str, Any]:
    ...
    profile_dir = Path(str(getattr(feedback, "user_profile_dir", "~/.vocal_subtitle/profiles"))).expanduser()
    ...
    if not path.exists():
        result.update({"status": "fallback_base", "fallback": True, "reason": "profile_missing"})
        return result
    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8")) or {}
        overrides = payload.get("overrides", {}) if isinstance(payload, dict) else {}
        digest = hashlib.sha256(
            yaml.safe_dump(overrides, sort_keys=True, allow_unicode=True).encode("utf-8")
        ).hexdigest()
        result.update({
            ...
            "profile_version": payload.get("updated_at") if isinstance(payload, dict) else None,
        })
    except Exception as exc:
        result.update({"status": "fallback_base", "fallback": True, "reason": f"profile_load_failed:{type(exc).__name__}"})
    return result
