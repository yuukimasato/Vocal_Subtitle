...
def output_format_option(function):
    ...
    return click.option(
        "--format", "-f", "output_format", default="srt",
        type=click.Choice(["srt", "vtt", "ass"]), help="输出字幕格式",
    )(function)
...
def device_option(function):
    ...
    return click.option(
        "--device", "-d", default=None, type=click.Choice(["cuda", "cpu"]),
        help="推理设备（默认自动检测）",
    )(function)
...
def language_option(function):
    ...
    return click.option("--language", "-l", default=None, help="语言代码 (zh/en/ja/...)")(function)
