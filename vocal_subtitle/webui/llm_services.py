...
class LLMModelService:
    ...
    def fetch_models(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        for provider in LLM_PROVIDERS.values():
            if provider["base_url"].rstrip("/") == clean_url and provider.get(
                "default_models"
            ):
                models = [
                    {"id": model, "owned_by": provider["name"]}
        ...
        if not api_key:
            raise LLMModelLookupError(
                400, "未提供 API 密钥且 API 地址需要认证，请先输入密钥后重试"
            )
        raise LLMModelLookupError(
            502, "获取模型列表失败：API 不可达或密钥无效。将使用预设默认模型。"
        )
