(function() {
  ...
  const QualityUI = {
    initialized: false, taskId: null, tasks: [],
    init() {
      if (this.initialized) return;
      this.initialized = true;
      document.getElementById('quality-task-select')?.addEventListener('change', (event) => this.load(event.target.value));
      document.getElementById('quality-refresh')?.addEventListener('click', () => this.load(this.taskId, true));
      document.addEventListener('workspace:task-change', (event) => { if (Workspace.current === 'quality' && event.detail.taskId !== this.taskId) this.load(event.detail.taskId); });
    },
    async refresh() {
      this.init();
      try {
        if (!this.tasks.length) {
          const data = await VocalSubtitleApi.getHistory(50, 0); this.tasks = data.items || [];
          const select = document.getElementById('quality-task-select');
          if (select) select.innerHTML = '<option value="">选择任务</option>' + this.tasks.map((task) => '<option value="' + esc(task.id) + '">' + esc(task.input_file_name || task.id) + '</option>').join('');
        }
    ...
    async load(taskId, force = false) {
      ...
      this.taskId = taskId; Workspace.setTask(taskId);
      const container = document.getElementById('quality-report-content'); if (container) container.innerHTML = '<div class="workspace-panel workspace-loading">加载质量报告...</div>';
      try { const data = await VocalSubtitleApi.getQualityReport(taskId); this.render(data, force); }
      catch (error) { this.renderError(error); }
    },
    render(data) {
      ...
      document.getElementById('quality-report-meta').textContent = '来源: ' + (data.report_source === 'run_report' ? 'run_report' : '历史摘要') + ' · ' + (data.report_schema_version || 'run-report-v1');
