...
class ReviewScheduler:
    ...
    def __init__(self, config: ReviewSchedulerConfig | None = None):
    ...
    def schedule(
        ...
        *,
        ...
        for candidate in targets:
            if not current or self._can_merge(
                current[-1], candidate, timeline=timeline
            ):
                current.append(candidate)
                continue
            windows.append(
                self._make_window(current, timeline=timeline, index=len(windows))
            )
            current = [candidate]
        if current:
            windows.append(
    ...
    def review(
        ...
        engine: ReviewEnginePort | None = None,
        *,
        ...
        diagnostics["degraded"] = any(
            item.get("degraded") for item in diagnostics["windows"]
        )
    ...
    def _can_merge(
        ...
        *,
        ...
        if (
            ...
            > self.config.max_group_duration
        ):
            return False
        ...
        if previous_clip and current_clip and previous_clip != current_clip:
            return False
        if timeline is not None and self._crosses_hard_gap(
            previous.end, current.start, timeline
        ):
            return False
        return True
    ...
    def _make_window(
        ...
        *,
        ...
        clip_ids = {
            item.physical_clip_id for item in candidates if item.physical_clip_id
        }
        return ReviewWindow(
