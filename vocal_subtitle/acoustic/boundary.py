...
def find_directional_boundary(
    t: float, skeleton: list[tuple[float, float]], boundary_type: str
...
def boundary_confidence(event: object, boundary_type: str) -> float | None:
    words = list(getattr(event, "words", []) or [])
    if words:
        value = getattr(
            words[0] if boundary_type == "start" else words[-1], "confidence", None
        )
...
def preserve_reliable_asr_boundary(
    event: object, boundary_type: str, config: object
) -> bool:
    words = list(getattr(event, "words", []) or [])
    confidence = boundary_confidence(event, boundary_type)
    return bool(
        words and confidence is not None and confidence >= config.confidence_threshold
    )


def record_boundary_diagnostic(
    report: dict,
    event: object,
    boundary_type: str,
    action: str,
    ...
    report.setdefault("boundary_diagnostics", []).append(
        {
            ...
            "candidate_time": round(float(candidate_time), 6)
            if candidate_time is not None
            else None,
            "distance_ms": round(float(distance) * 1000, 3)
