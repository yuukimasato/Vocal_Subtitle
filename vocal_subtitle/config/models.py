"""Configuration dataclasses — all sub-configs and PipelineConfig itself.

These are pure data models with defaults. No filesystem I/O, YAML loading,
environment reads, or model construction happens in this module.
"""
...
class GlobalASRConfig:
    ...
    # Direct ``PipelineConfig()`` remains compatible with legacy callers;
    # shipped YAML profiles explicitly select evidence mode.
    mode: str = "production"  # evidence | production
    direct_max_duration_seconds: float = 180.0
    macro_window_seconds: float = 45.0
    macro_overlap_seconds: float = 2.0


@dataclass
class ContextReASRConfig:
    """异常片段上下文重识别和局部交叉复核配置。"""

    enabled: bool = True
    left_context_seconds: float = 0.8
    right_context_seconds: float = 0.8
    max_window_seconds: float = 15.0
    max_group_seconds: float = 10.0
    min_duration_trigger: float = 0.8
    cps_warning_threshold: float = 15.0
    cps_high_risk_threshold: float = 20.0
    primary_engine: str = "faster-whisper"
    primary_model: str = "large-v3"
    secondary_engine: str = "whisper-cpp"
    secondary_model: str = "medium"
    secondary_on_conflict: bool = True
    secondary_on_training_phrase: bool = True
    version: str = "context-reasr-v1"
    cross_engine_policy_version: str = "cross-engine-v1"
...
class ASRConfig:
    ...
    context_reasr: ContextReASRConfig = field(default_factory=ContextReASRConfig)
