...
@click.option(
    "--asr-model", default=None, help="Whisper 模型 (large-v3 / medium / small / tiny)"
)
@click.option(
    "--primary-engine",
    type=click.Choice(["auto", "funasr", "qwen", "faster-whisper", "whisper-cpp"]),
...
@click.option(
    "--speaker-fusion",
    type=click.Choice(["auto", "embedding", "dual"]),
    default=None,
    help="说话人融合模式",
)
@click.option(
    "--global-diarization-model",
    type=click.Choice(["auto", "none", "community-1", "diarization-3.1"]),
    default=None,
    help="全局 diarization 模型",
)
@click.option(
    "--speaker-diarization-scope",
    type=click.Choice(["global", "hierarchical"]),
    default=None,
    help="全局 turns 或全局加逐字幕局部精修",
)
@click.option(
    "--local-speaker-refinement",
    type=click.Choice(["off", "embedding", "full"]),
    default=None,
    help="逐字幕局部换人检测级别",
)
@click.option(
    "--speaker-role/--no-speaker-role", default=None, help="启用/禁用 LLM 角色标注"
)
...
def run(
    ...
    engine_pair_policy: str | None,
    asr_path: str | None,
    llm_optimize: bool | None,
    diarization: bool | None,
    expected_speakers: int | None,
    speaker_fusion: str | None,
    global_diarization_model: str | None,
    speaker_diarization_scope: str | None,
    local_speaker_refinement: str | None,
    speaker_role: bool | None,
    skip_separation: bool,
    skeleton_mode: bool | None,
    export_skeleton_segments: bool | None,
    export_skeleton_dir: str | None,
    verbose: bool,
):
    """处理单个音频文件，生成字幕
