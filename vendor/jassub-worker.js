// 上游 ASSRenderer 构造函数用 .catch(console.error) 吞掉异步初始化错误并 resolve(undefined)，
// 经 abslink CONSTRUCT 应答路径会被二次包装成误导性的 "Unserializable return value"；
// 这里改为透传，让真实错误能通过 ready 的 rejection 到达主线程。
...
async function Module(moduleArg = {}) {
  ...
  var PThread = { unusedWorkers: [], tlsInitFunctions: [], pthreads: {}, init() {
    ...
    var worker;
    // blob worker 场景：blob: URL 不能相对解析，但它本身就是可再 spawn 的完整模块脚本
    worker = new Worker((() => {
      try {
        return new URL("jassub-worker.js", import.meta.url);
      } catch {
  ...
  function loadModule() {
    ...
    // blob worker（standalone 内嵌）的 import.meta.url 是 blob: URL，无法相对解析；
    // 回退 moduleArg.__url（JASSUB 构造时传入的 wasm 地址）。fetch 通常已被劫持，此回退保证语义完整。
    instantiatePromise = WebAssembly.instantiateStreaming(fetch((() => {
      try {
        return new URL("jassub-worker.wasm", import.meta.url);
      } catch {
        return moduleArg.__url;
      }
    })()), imports).then((output) => {
...
// dist/worker/renderers/2d-renderer.js
...
// dist/worker/renderers/webgl1-renderer.js
var VERTEX_SHADER = (
  /* glsl */
  `
...
// dist/worker/renderers/webgl2-renderer.js
var VERTEX_SHADER2 = (
  /* glsl */
  `#version 300 es
...
var ASSRenderer = class {
  ...
  debug = false;
  constructor(...args) {
    return this[constructor](...args).catch((e) => { console.error(e); throw e; });
  }
