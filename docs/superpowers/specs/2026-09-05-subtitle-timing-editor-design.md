...
- 媒体加载（本地视频/音频文件，`<video>` 原生元素）与播放控制（播放/暂停/停止、逐帧步进、倍速、fps 选择、循环当前句）；
...
- ASS 样式解析与视频画面样式渲染预览（视频 overlay 仅白字黑边纯文本）；
...
4. 唯一第三方代码为 wavesurfer.js v7（BSD-3-Clause，katspaugh 及贡献者），以 vendor 文件引入并保留其 LICENSE 与版权声明，商用友好；不引入 Video.js（原生 `<video>` 足够）。

## 3. 技术形态

零构建：原生 ES Modules + 静态文件，无 npm 依赖、无打包步骤。本地预览需经 HTTP（ES Modules 不支持 `file://`），用 `python3 -m http.server` 或任意静态服务器指向本目录。
...
tools/subtitle-editor/
  ...
  js/
    ...
    format/time.js        # 时间解析/格式化（SRT/VTT/ASS/显示用）
    format/srt.js         # SRT 解析/序列化
    format/vtt.js         # WebVTT 解析/序列化
    format/ass.js         # ASS/SSA 骨架式解析/序列化
    format/index.js       # 格式嗅探与分发
    ui/player.js          # 播放控制条、试听窗口、字幕 overlay
    ui/waveform.js        # wavesurfer 封装 + 区块同步 + 降级视图
    ui/cue-list.js        # 字幕表格与行内编辑
    ui/toolbar.js         # 顶栏、文件打开、拖放、导出、草稿
  vendor/
    wavesurfer.esm.js     # BSD-3-Clause，保留上游文件
    regions.esm.js
    LICENSE
...
```js
{ cues, selectedId, editingId, duration, fps: 25, follow: true,
  mediaName, subtitleName, subtitleFormat, subDoc, dirty }
```

`cue = { id, start, end, text }`，秒为单位的 float；`cues` 恒按 start 稳定排序。ASS 额外携带 `cue.meta`（字段序、其余字段原值、骨架行号）。

数据流：文件打开 → 解析器产出 `{cues, doc}` → store → 三个订阅方（波形同步区块 / 列表重渲染 / 工具栏状态）。所有字幕变更只能走 `actions.js`，先推撤销快照再写 store 并派发 `cues` 事件。播放进度由 `player.js` 的 rAF 循环直接驱动时间显示、overlay、列表高亮与试听窗口，不进 store。

## 5. 交互与布局

上方视频（含可开关的字幕文本 overlay）、下方波形轨道 + 播放控制条、右侧字幕列表；<1100px 列表折叠到底部。波形与 `<video>` 绑定同一媒体元素。
...
- 降级：波形解码失败显示错误条 + 静态刻度区块视图（点击 seek、随 cues 重绘），编辑经列表完成。
...
主键盘数字排镜像同一套动作（无小键盘设备）；其余：Space 播放暂停、←/→ seek 5s（Shift 1s）、`,`/`.` 逐帧、N 插入、Delete 删除、Ctrl+D 拆分、Ctrl+M 合并、L 循环当前句、F 跟随、Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y 撤销重做、Ctrl+S 按当前格式导出。输入框聚焦时仅放行编辑器自身按键。
...
- ASS/SSA：骨架式——非 Dialogue 行原样保留；`[Events] Format:` 定义字段序（缺失时用 v4+ 常见默认序）；Dialogue 按字段拆分，Text 为最后一段（允许含逗号）；`\N`/`\n` ↔ 换行双向转换；删除的句对应行整体移除；新增句以默认字段追加。
- 嗅探：优先扩展名，失败按内容特征（`WEBVTT` 头 / `-->` 时间行 / `[Script Info]`）。

## 8. 错误处理

媒体加载失败、字幕解析失败、音频解码失败均给出可见且可操作的提示；时间输入非法拒绝提交并还原；草稿写 localStorage 失败静默降级；`beforeunload` 在有未导出修改时提醒。不在本地存储任何敏感信息。

## 9. 测试与验收

- 解析器单测（`node --test`）：time/SRT/VTT/ASS 的解析、容错、错误行号、往返一致性（含 ASS 骨架保真、删除句、文本逗号、`\N` 转换）与嗅探。
- 浏览器 smoke（本地静态服务）：打开媒体出波形、解析示例字幕、区块拖拽改时间回写列表、小键盘试听/微调、导出产物与解析往返一致、断开音频解码走降级视图。
- 验收：全程零构建可运行；不修改 `tools/subtitle-editor/` 之外任何文件（spec 与本目录除外）；README 含使用方法、BSD-3-Clause 声明与洁净室声明。
