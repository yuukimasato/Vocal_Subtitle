...
- 支持两条主线：全音频声纹身份线，以及包含全局/局部两级处理的层级式 diarization 线。
...
默认配置仍以 SpeechBrain ECAPA 为基础。全局模型模式为 `auto` 时，只有本地全局模型完整可用才启用线路四并与线路一融合，否则自动回到 ECAPA 单路。

### 3.2 说话人数与融合模式

- `expected_speakers` 留空：自动估计。
- `expected_speakers=N`：将 N 同时传给全局模型和嵌入聚类；全局模型使用 `min_speakers=N,max_speakers=N`，嵌入聚类以 N 为目标簇数。
- `fusion_mode=auto`：本地有全局模型时启用线路四并与线路一融合，否则 ECAPA 单路。
...
- `diarization_scope=global|hierarchical`：选择只做全局 turns，或启用全局 turns 加逐字幕局部精修的线路四。
- `local_refinement=off|embedding|full`：控制线路四的局部精修强度；`off` 等同于 `global`。
...
### 4.2 线路四：层级式全局-局部 diarization 线

线路四由两个有先后关系的 pass 组成，使用同一个已加载的 Community-1 或 Diarization 3.1 模型。它不是两个相互独立的 diarization 结果投票，而是“全局结果作为局部精修先验”的层级流程。

#### 全局 pass

对完整音频运行一次。模型只加载一次，不能为每条字幕重新初始化或独立推理完整上下文。
...
以全局 turns 和字幕事件为先验，对每一条候选字幕执行局部换人复核。局部 pass 不独立创建新的 speaker 编号，只能补充或修正全局 pass 的边界。
...
4. `embedding` 模式使用线路一的声纹相似度和局部变化分数辅助复核。
5. `full` 模式额外复用线路四的全局模型，对字幕前后 padding 窗口做局部复核。
6. 通过最小片段时长、停顿长度、speaker 分数和证据支持数进行质量门控。
7. 切点优先选择 word boundary；没有词时间戳时，跨 speaker 的事件标记为 `unknown`，不将整句话强行归给一人。

局部 pass 的职责是发现“全局 turn 没有覆盖或字幕合并隐藏的内部换人”，不是替代全局 pass，也不能脱离全局 speaker 数量约束独立聚类。
...
使用重叠时长矩阵把线路四的全局标签映射到线路一 cluster：
...
- 两条主线映射后一致：使用该 speaker，状态为 `fused`。
- 线路四全局 pass 检测到边界且局部 pass 或线路一也支持变化：确认切分。
- 只有局部 pass 支持边界：要求线路一身份变化、高置信度、合理停顿和最小片段时长；否则保留原事件并记录冲突。
- 线路一与线路四 speaker 身份冲突且无法通过映射/分数解决：speaker 为 `unknown`，不强行选择。
- 线路四局部精修发现边界后，最终事件不得再被 LLM 合并器跨 speaker 合并。
...
1. 解析 fusion/global/embedding/scope 配置。
2. 运行线路一；如果启用线路四且全局模型可用，先运行全局 pass。
3. 对线路一和线路四全局结果做 canonicalization 和 speaker mapping。
4. 在 ASR word timestamps 可用后，按线路四 scope 运行局部 pass。
5. 构建 atomic spans，替换事件级滑窗聚类的最终赋值逻辑。
6. 将两条主线的结果注入事件并运行现有物理边界、字幕构建和导出流程。
...
--speaker-diarization-scope [global|hierarchical]
```
...
- 全局模型缺失、pyannote 依赖缺失、HF 协议/token 失败：记录脱敏原因，`auto` 进入 ECAPA；`dual` 标记 degraded；`hierarchical` 降级为线路一并标记 degraded。
- embedding 加载/提取失败：如果全局模型可用则使用全局线，否则 speaker 保持 unknown。
- 两条主线质量不足：禁止 gap alternation 和字幕序号推断。
...
- 线路四全局 speaker ID 与 embedding cluster 的一一映射不依赖原始数字编号。
- 局部字幕事件在词边界处分裂，并保留 source word IDs、物理跨度和时间约束。
- 线路四全局 pass 与局部 pass 的先验约束、补充边界、冲突和低置信度结果。
...
5. 全局模型不可用时 ECAPA 路径可用；线路四不可用时字幕仍可导出且 speaker 为 unknown。
...
3. 实现线路四全局 pass 与线路一 embedding cluster 的对齐。
4. 实现线路四局部 pass、原子片段切分和两条主线融合。
